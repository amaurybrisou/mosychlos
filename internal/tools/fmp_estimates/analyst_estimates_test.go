...
func TestAnalystEstimatesProvider_New(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			provider, err := NewAnalystEstimatesProvider(c.apiKey, c.cacheDir, sharedBag)
...
func TestAnalystEstimatesProvider_ToolInterface(t *testing.T) {
	sharedBag := bag.NewSharedBag()
	provider, err := NewAnalystEstimatesProvider("test-key", "/tmp", sharedBag)
	if err != nil {
...
func TestAnalystEstimatesProvider_Run_Integration(t *testing.T) {
	...
	provider, err := NewAnalystEstimatesProvider(apiKey, "/tmp", sharedBag)
	if err != nil {
		t.Fatalf("failed to create provider: %v", err)
	}
